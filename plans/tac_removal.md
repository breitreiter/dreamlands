---
kind: plan
title: .tac File Removal (fold to inline .enc checks)
state: active
created: 2026-04-27
updated: 2026-05-12
status: current
touches:
  files:
    ...
    - text/encounters/plains/tier1/Road Toll Chase.tac
    - text/encounters/plains/tier2/Collapsed Earthwork Traverse.tac
    - text/encounters/plains/tier2/The Conscripts Combat.tac
    - text/encounters/plains/tier2/The Courier Conflict.tac
    - text/encounters/plains/tier2/The Requisition Line Conflict.tac
    - text/encounters/plains/tier2/The Scavengers Fight.tac
    - text/encounters/plains/tier2/The Scavengers Chase.tac
    - text/encounters/plains/tier3/The Gentle Giant Escape.tac
    - text/encounters/plains/tier3/The Gentle Giant Sneak.tac
    - text/encounters/plains/tier3/The Passage Combat.tac
    - text/encounters/plains/tier3/The Passage Door.tac
    - text/encounters/plains/tier3/The Passage Traverse.tac
    - lib/Tactical/
  features: [combat, tactical-removal, single-check]
provenance:
  author: migration:M-001
---

> Migration note (M-001, 2026-05-12): **Partially superseded by RPS combat
> pivot.** Track F (fold each `.tac` into an inline `+check / @if / @else` on
> the parent `.enc` choice) is **still the active plan** — 14 `.tac` files
> remain on disk, `lib/Tactical/` still compiles, and the work is unstarted.
> Single-check combat is the live shape for these narrative resolutions.
> Track P (port to `.cmb` combat encounters) is **moot**: the `.cmb` format
> never landed; the RPS combat system replaced it. For Road Toll Fight and
> Sentinel 7-C (the two Track P candidates), use the inline-check fold like
> any Track F file, or — if they deserve a real RPS combat — author them
> against `lib/Combat/` directly (no separate file format).
