---
kind: plan
title: Google OAuth + Session Reconnect
state: exploring
created: 2026-02-25
updated: 2026-02-25
status: current
touches:
  files:
    ...
    - ui/web/src/screens/Splash.tsx
  features: [auth, session-reconnect, google-oauth]
provenance:
  author: migration:M-001
---

> Migration note (M-001, 2026-05-12): No implementation yet — current auth is
> still restore-codes only (see [[web_architecture]] "Auth" section). Plan stays
> in `exploring`. The "guest mode" fallback noted below keeps the existing
> restore-code path viable as a coexistence strategy if/when this work picks up.
