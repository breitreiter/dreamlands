---
kind: plan
title: CLI Play Harness — Map + Encounters
state: shipped
created: 2026-02-21
updated: 2026-05-12
status: current
touches:
  files:
    - lib/Map/MapRenderer.cs
    - lib/Encounter/BundleLoader.cs
    - lib/Orchestration/GameSession.cs
    - lib/Orchestration/Movement.cs
    - lib/Orchestration/EncounterSelection.cs
    - lib/Orchestration/EncounterRunner.cs
    - ui/cli/
  features: [cli, orchestration, play-harness]
provenance:
  author: migration:M-001
---

> Migration note (M-001, 2026-05-12): All four phases shipped. Phase 0 verified
> — `lib/Map/MapRenderer.cs` exists. Phases 1-3 (BundleLoader, Orchestration lib,
> CLI app at `ui/cli/`) all present. CLI is now an HTTP client against GameServer
> rather than the original direct-link console (see [[web_architecture]]), but
> the Orchestration library carved out here is the foundation both the CLI and
> the web client now use.
