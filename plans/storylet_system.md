---
kind: plan
title: Storylet System Migration
state: exploring
created: 2026-03-12
updated: 2026-03-12
status: current
touches:
  ...
  features: [encounter-selection, storylets]
provenance:
...
> Migration note (M-001, 2026-05-12): Step 1 (encounter-level `[requires]`)
> shipped — encounters now parse and store a `Requires` field, and
> selection filters by it. Steps 2-6 (priority tiers, richer selection
> logic, additional trigger points) remain unbuilt. Plan stays in
> `exploring` until next decision point.
