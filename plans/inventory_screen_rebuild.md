---
kind: plan
title: Inventory Screen Rebuild (three-column layout)
state: shipped
created: 2026-02-28
updated: 2026-05-12
status: current
touches:
  files:
    ...
    - lib/Rules/SkillFlavor.cs
  features: [ui, inventory, mechanics-panel, character-panel]
provenance:
...
> Migration note (M-001, 2026-05-12): Verified shipped. The four phases all
> landed in `ui/web/src/screens/Inventory.tsx` (now ~423 lines), with
> `CharacterPanel` and `MechanicsPanel` implemented inline rather than as
> separate components. Server enrichment (MechanicsInfo, ItemInfo type/cost,
> ConditionInfo, SkillInfo) all present in `GameResponse.cs`. Tabs present.
> Kept as-shipped for historical record and the rationale captured below.
