---
kind: plan
title: Card-Based Encounter System (abandoned, predecessor to RPS)
state: abandoned
created: 2026-03-29
updated: 2026-05-15
status: superseded
touches:
  files:
    - lib/Tactical/
    - text/encounters/plains/tier1/Road Toll Fight.tac
    - text/encounters/plains/tier2/The Scavengers Fight.tac
  features: [combat, tactical, cards, abandoned]
supersedes: []
provenance:
  author: migration:M-001
---

> Migration note (M-001, 2026-05-15): This documents the Q1–Q2 2026
> card-based encounter system (Combat + Traverse variants with Momentum /
> Spirits / Resistance / Timers). It was the original implementation of
> `.tac` files and `lib/Tactical/`. Abandoned in two steps:
> first replaced by d20 combat (April 2026, also abandoned — see
> [[d20_combat_attempt]]), then by RPS combat (May 2026, current — see
> [[rps_combat_pivot]] and [[super_rps]]).
>
> **Residue still in repo:** 14 `.tac` files in `text/encounters/`,
> `lib/Tactical/` (parser + bundle still compiles but takes no input
> from itemdef cards). The fold from `.tac` to inline `.enc` checks is
> tracked in [[tac_removal]].
>
> **Supporting design docs still in `project/design/encounter-redesign/`**:
> - `overview.md` (this body)
> - `combat.md` — combat-variant mechanics
> - `traverse.md` — traverse-variant mechanics
> - `card_archetypes.md` — opening-card archetypes
> - `card_lists.md` — weapon/skill card inventories by stat
> - `deckbuilding.md` — deck-assembly framework
> - `fun_deck.md` — sequential-timers/cancels variant
> - `mma-inspired.md` — the "combat sports" state-property dead-end
>   (Initiative/Control/Spirits/Resistance verbs — Convert/Destabilize/
>   Exploit/Press). This was the precursor framing.
> - `tac_skeleton_gen.md` — LLM-assisted .tac generation tool design
> - `old/aspect-based.md`, `old/aspects.md`, `old/journey_alt.md`,
>   `old/journey_hazards.md`, `old/journey_hazards_blocks.md` —
>   even-earlier brainstorming (already retired before .tac shipped)
>
> All listed as raw-material in [[MIGRATION_LOG]] — recommend dropping
> once the .tac removal pass completes.
