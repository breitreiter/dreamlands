---
kind: learning
title: Original Dungeon Plan (superseded by arc system)
created: 2026-02-21
updated: 2026-02-21
status: stale
touches:
  files:
    ...
    - mapgen/DungeonPlacer.cs
    - text/encounters/arcs/
  features: [dungeons, arcs, placement, content-pipeline]
topics: [dungeons, arcs, placement, history]
provenance:
  author: migration:M-001
---

> Migration note (M-001, 2026-05-12): Dungeons were replaced by **biome-grounded
> narrative arcs** in 2026-03 (see `memory/project_arcs_structure.md`). The
> placement infrastructure described here is still live — `dungeons_roster.yaml`
> drives placement via `DungeonPlacer.cs`, and 20 arcs are now placed instead of
> dungeons. The "dungeon as one-and-done navigable adventure" framing here is
> retired; arcs fire encounters across the biome (settlements, overworld) rather
> than at a single POI. The doc's content-pipeline thinking (roster → placement
> → authoring → runtime) is what actually happened, with the framing replaced.
>
> **Why retained as `learning`:** Captures the design rationale behind dead-end
> placement, tier difficulty, decal-per-dungeon visual identity, and one-and-done
> tracking — all of which carried forward into the arc system. The "open questions"
> (mid-chain flee, combat resolution, naming) were largely resolved during the
> arc pivot. Useful when reasoning about why the placement algorithm is shaped
> the way it is.
