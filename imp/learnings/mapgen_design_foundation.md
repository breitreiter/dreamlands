---
kind: learning
title: MapGen Design Foundation — World as Graph
created: 2026-02-21
updated: 2026-02-21
status: current
touches:
  files:
    ...
    - lib/Map/Node.cs
    - lib/Map/Region.cs
  features: [mapgen, world-graph, terrain, rivers]
topics: [mapgen, architecture, world-graph]
provenance:
  author: migration:M-001
---

> Migration note (M-001, 2026-05-12): This is the design foundation that guided
> mapgen implementation. The system is live and behaves as described — graph
> (not grid), 3-mile nodes, terrain types (Plains/Forest/Hills/Mountains/Swamp/
> Riverland; no Ocean/Coast — see [[no_coastline]] note in CLAUDE.md memory),
> adjacency rules, rivers as edge-position barriers, hybrid generation
> (elevation zones + random-walk rivers + terrain splats). Useful as **onboarding
> context** and to understand *why* the world is shaped this way. The "open
> questions" at the bottom are mostly resolved by the live implementation.
>
> **Why retained as `learning`:** Implementation-derivable details aside, the
> map-as-graph (not grid) framing and the spatial-coherence requirements are
> design intent that's invisible from reading code. Also the rivers-as-
> edge-position model is non-obvious and the rationale is captured here.
