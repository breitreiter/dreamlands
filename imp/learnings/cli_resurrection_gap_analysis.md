---
kind: learning
title: CLI Resurrection Gap Analysis (historical)
created: 2026-02-21
updated: 2026-03-12
status: stale
touches:
  files:
    ...
    - lib/Orchestration/
    - ui/cli/
  features: [cli, architecture, library-boundaries]
topics: [architecture, library-boundaries, history]
provenance:
  author: migration:M-001
---

> Migration note (M-001, 2026-05-12): This is the pre-flight discovery doc that
> led to the [[cli_play_harness]] build order. Most of the identified gaps have
> been closed: `ItemDef.cs`/`ItemInstance.cs`/`PlayerState.cs` exist, balance is
> compiled in (`BalanceData.Default`, not YAML), the Game library is built, the
> Orchestration layer was carved out, and the CLI lives at `ui/cli/`. Kept as a
> learning for the **library-boundaries rubric** (Rules = vocabulary; Game = state
> + mutation), the explicit retirement of the water-economy GameState, and the
> "what belongs in /lib vs what's UI" framing — those still apply.
>
> **Why retained as `learning`:** The contradictions section captures decisions
> that aren't documented anywhere else (TimePeriod won over TimeOfDay; the old
> water-centric GameState was retired; inn replaces "entertainment" in balance
> data; guild outpost replaces generic "storage"). Useful context if anyone
> wonders why old constructs vanished.
> **How to apply:** Read when introducing new libs that need to land somewhere
> in the lib/ tree, or when reasoning about Rules-vs-Game boundary disputes.
