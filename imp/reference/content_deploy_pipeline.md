---
kind: reference
title: Content Deploy Pipeline (encounter hot-reload)
created: 2026-03-15
updated: 2026-03-15
status: current
touches:
  files:
    - server/GameServer/GameData.cs
    - text/encounter-tool/EncounterCli/PushCommand.cs
    - deploy.sh
  features: [deployment, encounters, hot-reload]
provenance:
  author: migration:M-001
subject: How encounter content hot-deploys to Azure Functions without a full API restart
---

> Migration note (M-001, 2026-05-12): All four changes shipped. `GameData.ReloadBundle()`
> exists, the `/api/ops/reload-bundle` endpoint is live, and `encounter push` is the
> default content-iteration loop. Reclassified plan → reference.
