---
kind: reference
title: Web Architecture — Thin Server, Dumb Clients
created: 2026-02-21
updated: 2026-02-21
status: current
touches:
  files:
    ...
    - server/GameServer/GameStore.cs
    - lib/Orchestration/
    - ui/web/src/
    - ui/cli/
  features: [architecture, client-server, scoreboard, restore-codes, map-rendering]
provenance:
  author: migration:M-001
subject: Macro web architecture — server owns state, clients render structured data; restore-code auth; multi-client model
---

> Migration note (M-001, 2026-05-12): Originally titled "Web UI Plan". Reclassified
> plan → reference — the architecture described (thin server, dumb clients,
> mode-driven response, restore-code auth, Cosmos in prod / file store in dev,
> CLI as alternate client) is the shipped state. See also
> [[client_server_architecture]] for the API contract details and [[web_tech_stack]]
> for stack choices.
