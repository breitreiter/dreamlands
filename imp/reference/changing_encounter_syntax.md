---
kind: reference
title: How to Extend the Encounter Syntax
created: 2026-02-21
updated: 2026-02-21
status: current
touches:
  files:
    ...
    - lib/Encounter/Choice.cs
    - lib/Encounter/Encounter.cs
    - lib/Rules/ActionVocabulary.cs
    - text/encounter-tool/EncounterCli/BundleCommand.cs
    - text/encounter-tool/EncounterCli/CheckCommand.cs
    - text/encounter-tool/EncounterCli/EncounterPostProcessor.cs
  features: [encounter-format, parser, vocabulary]
provenance:
  author: migration:M-001
subject: Architecture overview and touch-points for adding new .enc syntax features
---
...
- **Format spec** (formerly `text/notes/ENCOUNTER_FORMAT.md`; current canonical is [[encounter_format]]) — Human-readable grammar reference.
...
2. **`rules/encounter_mechanics.md`** — Add a row to the "Game commands" table.
...
3. **`rules/encounter_mechanics.md`** — Add a row to "Argument types".
...
3. **`rules/encounter_mechanics.md`** — Add a row to the "Conditions" table.
...
4. **`rules/encounter_format.md`** — Document in sections 2, 3, and 6.
